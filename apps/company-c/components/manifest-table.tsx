...
export function ManifestTable({ vehicles }: { vehicles: Vehicle[] }) {
  return (
    <div className="border border-line bg-surface">
      <div className="hidden border-b border-line bg-elevated px-4 py-2 md:grid md:grid-cols-[minmax(0,1fr)_8rem_9rem_7rem] md:gap-4">
        {["Model", "Type", "Capacity", "Status"].map((head) => (
        ...
        {vehicles.map((vehicle) => (
          <li key={vehicle.slug} className="border-b border-line last:border-b-0">
            <Link
              ...
              className="group grid gap-2 px-4 py-4 transition-colors duration-150 hover:bg-elevated md:grid-cols-[minmax(0,1fr)_8rem_9rem_7rem] md:items-center md:gap-4"
            >
